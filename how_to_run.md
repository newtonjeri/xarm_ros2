...
The script will prompt you to choose between running the simulation or the real robot, also you will be asked whether you want to connect to unity, if so you have the option to use the default ip_address or input a new ip_address if the machine running ROS 2 ip has changed

You can check the machines IP by running the following command
```bash 
hostname -I
```
### run simulated xarm7
- Input "sim" to run the simulated xarm7 robot
```bash
...
### run real xarm7
- Input "real" to run the real xarm7 robot
```bash
# Navigate to the shared folder
cd /home/shared_folder/;
# Execute the script to run simulated xarm7
