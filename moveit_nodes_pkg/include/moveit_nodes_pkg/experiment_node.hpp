#ifndef __EXPERIMENT_NODE_HPP__
#define __EXPERIMENT_NODE_HPP__
...
namespace experiment
{
    ...
    public:
        ExperimentNode(rclcpp::Node::SharedPtr &node, const std::string &group_name) : node_(node)
        {
            init(group_name);
        }
        ...
        void planToTargetPose(const geometry_msgs::msg::Pose &target_pose, bool stop_moving = false)
        {
            if (stop_moving)
            {
                move_group->stop();
                RCLCPP_WARN(rclcpp::get_logger("experiment_node"), "Robot motion stopped!!");
                return;
            }
            ...
            if (!success)
            {
                RCLCPP_ERROR(rclcpp::get_logger("experiment_node"),
        ...
        void gripperOpenAndClose(const std::vector<double> &target_joint_positions)
        {
            ...
            // {
            //     RCLCPP_ERROR(rclcpp::get_logger("experiment_node"),
            //                  "Pre-plan -> Fail Failed to set the joint value target. Please ensure target is within gripper joints range");
            //     return;
            // }
            ...
            {
                RCLCPP_ERROR(rclcpp::get_logger("experiment_node"),
                             "Post-plan Fail -> Failed to plan a path to the target pose. Please ensure target is within gripper joints range");
                return;
            }
            else
            {
            ...
            const std::vector<double> direction)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("experiment_node"), "x = %f, y = %f, z = %f, ox = %f, oy = %f, oz = %f, ow = %f",
            ...
            if (fraction < 0.90)
            {
                RCLCPP_WARN(rclcpp::get_logger("experiment_node"), "Cartesian path for approach not fully planned, fraction: %f", fraction);
            ...
            [this](const moveit::planning_interface::MoveGroupInterface::Plan &trajectory)
        {
            ...
            moveit_visual_tools->publishTrajectoryLine(trajectory.trajectory, jmg);
