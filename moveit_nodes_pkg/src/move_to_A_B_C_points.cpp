...
int main(int argc, char* argv[])
{
 ...
 auto const target_pose_A = []{
  ...
  if (success_A)
  {
    draw_trajectory_tool_path(plan_A.trajectory);
    moveit_visual_tools.trigger();
  ...
  if (success_B)
  {
    draw_trajectory_tool_path(plan_B.trajectory);
    moveit_visual_tools.trigger();
    prompt("Press 'next' in the RvizVisualToolsGui window to execute");
    draw_title("Executing");
    moveit_visual_tools.trigger();
  ...
  auto const target_pose_C = []{
    geometry_msgs::msg::Pose msg;
        msg.position.x = 0.3851;
        msg.position.y = -0.0912;
        msg.position.z = 0.01064;
        msg.orientation.x = 1.0;
        msg.orientation.y = 0.0;
        msg.orientation.z = 0.0;
        msg.orientation.w = 0.0;
  ...
  if (success_C)
  {
    draw_trajectory_tool_path(plan_C.trajectory);
    moveit_visual_tools.trigger();
    prompt("Press 'next' in the RvizVisualToolsGui window to execute");
    draw_title("Executing");
    moveit_visual_tools.trigger();
