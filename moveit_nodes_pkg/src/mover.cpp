  ...
  void xarm7UnityCallback(const Xarm7UnityTargetPoseToROS::SharedPtr msg)
  {
    if (!is_trajectory_)
    {
      ...
      try
      {
        ...
        sendPlanToUnity(motion_plan_.trajectory);
      }
...
geometry_msgs::msg::Pose rotatePose(const geometry_msgs::msg::Pose &pose, double roll, double pitch, double yaw) {
  ...
  void planPickAndPlace(const MoverServiceRequest::SharedPtr req, MoverServiceResponse::SharedPtr res)
  {
    try
    {
      ...
      if (pre_grasp_pose.trajectory.joint_trajectory.points.empty())
        return;

      // sendPlanToUnity(pre_grasp_pose.trajectory);
      auto previous_ending_joint_angles = pre_grasp_pose.trajectory.joint_trajectory.points.back().positions;
      ...
      if (grasp_pose.trajectory.joint_trajectory.points.empty())
        return;

      // sendPlanToUnity(grasp_pose.trajectory);
      previous_ending_joint_angles = grasp_pose.trajectory.joint_trajectory.points.back().positions;

      auto pick_up_pose = planTrajectory(req->pick_pose, req->robot_arm_rotation);
      if (pick_up_pose.trajectory.joint_trajectory.points.empty())
        return;

      // sendPlanToUnity(pick_up_pose.trajectory);
      previous_ending_joint_angles = pick_up_pose.trajectory.joint_trajectory.points.back().positions;

      auto place_pose = planTrajectory(req->place_pose, req->robot_arm_rotation);
      if (place_pose.trajectory.joint_trajectory.points.empty())
        return;

      sendPlanToUnity(place_pose.trajectory);
      res->trajectories.push_back(pre_grasp_pose.trajectory);
      res->trajectories.push_back(grasp_pose.trajectory);
      res->trajectories.push_back(pick_up_pose.trajectory);
      res->trajectories.push_back(place_pose.trajectory);
