...
int main(int argc, char **argv)
{
    ...
    std::vector<double> close_gripper(6, 0.85);
    ...
    // {
    //     /* Line Constraint test */
    //     RCLCPP_INFO(rccpp::get_logger("pick_and_place_node"), "PLANNING AND EXECUTING TO A TARGET ATTEMPT - %d", i+1);
    //     arm_node->planl_and_execute_to_joint_space_goal(home_joint_values);
    //     RCLCPP_INFO(rclcpp::get_logger("pick_and_place_node"), "Robot At Home POSE - 1");
    ...
    // arm_node->plan_and_execute_with_box_constraint(home_pose, pregrasp_pose, "link3", {0.2, 0.2, 0.2});
    arm_node->plan_and_execute_to_pose_goal(pregrasp_pose, false);
    ...
    /* Cartesian path planning */
    std::vector<double> direction = {0.0, 0.0, -0.18};
    iterations = 1;
    ...
    arm_node->plan_and_execute_to_pose_goal(preplace_pose, false);
