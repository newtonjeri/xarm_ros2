...
from niryo_moveit.msg import NiryoUnityTargetPoseToROS, NiryoROSJointsToUnity, NiryoUnityRobotJoints
...
def plan_pick_and_place(req):
    # print("Message received from Unity: {}, {}, {}".format(req.pick_pose, req.joints_input.joints, req.robotArmRotation))
...
def niryo_unity_callback(msg):
    global planning, motionPlan
    if not planning:
        planning = True
        # print("Message received from Unity: {}, {}, {}".format(msg.target_pose, msg.joints_input.joints, msg.robotArmRotation))
        motionPlan = plan_trajectory(move_group, msg.target_pose, msg.joints_input.joints, msg.robotArmRotation)
...
def moveit_server():
    ...
    global pub
    pub = rospy.Publisher('niryo_ROS_joints_to_unity', NiryoROSJointsToUnity, queue_size=10)
    rospy.Subscriber('niryo_unity_target_pose_to_ROS', NiryoUnityTargetPoseToROS, niryo_unity_callback)
    service = rospy.Service('niryo_moveit', MoverService, plan_pick_and_place)
    ...
    while not rospy.is_shutdown():
        if motionPlan:
            ...
            if motionPlan and motionPlan.joint_trajectory.points:
