...
# MTC Compatible Mode Switcher Node
#----------------------------------------------------------------#

import rclpy
from rclpy.node import Node
from xarm_msgs.srv import SetInt16, MTCTaskRequest  # Import both service messages
...
class MTCModeSwitcher(Node):
    def __init__(self):
        super().__init__('mtc_mode_switcher')

        self.process_exist: bool = False
        self.state_machine_state = 0  # Track state machine state (0=IDLE, 1=EXECUTING_TASK, 2=COMPLETED, 3=ERROR)
        self.state_names = {
            0: "IDLE", 1: "EXECUTING_TASK", 2: "COMPLETED", 3: "ERROR"
        }

        # Create subscribers 
        self.mode_subscriber = self.create_subscription(RobotMode, "/robot_mode", self.mode_callback, 10)
        self.state_machine_subscriber = self.create_subscription(UInt8, "/mtc_state_machine_state", self.state_machine_callback, 10)
        
        # Create service server for MTC task requests
        self.mtc_task_service = self.create_service(MTCTaskRequest, '/mtc_task_request', self.handle_mtc_task_request)
        ...
        self.get_logger().info("MODE: %s -- STATE-MACHINE: %s -- MTC Mode Switcher node started" % (
                              self.current_mode, self.state_names.get(self.state_machine_state, "UNKNOWN")))
        self.switch_processes(self.current_mode)

    def handle_mtc_task_request(self, request, response):
        """
        Handle MTC task requests and mode switching
        """
        # Check if this is a mode switch request
        if request.task_name == "mode_switch":
            if request.mode == 0:  # MOVEIT mode
                requested_mode = "MODE-MOVEIT"
            elif request.mode == 2:  # MANUAL mode
            ...
            if self.is_safe_to_switch_mode(requested_mode):
                ...
                response.message = f"Mode switched from {old_mode} to {self.current_mode}"
                self.get_logger().info("MODE: %s -- STATE-MACHINE: %s -- %s" % (
                                     self.current_mode,
                                     self.state_names.get(self.state_machine_state, "UNKNOWN"),
                                     response.message))
            else:
                response.success = False
                response.message = f"Mode switch to {requested_mode} denied - unsafe state"
                self.get_logger().warn("MODE: %s -- STATE-MACHINE: %s -- %s" % (
                                     self.current_mode,
                                     self.state_names.get(self.state_machine_state, "UNKNOWN"),
                                     response.message))
        else:
            # This is not a mode switch request, just acknowledge
            response.success = True
            response.message = f"MTC task request received: {request.task_name}"
            self.get_logger().info("MODE: %s -- STATE-MACHINE: %s -- MTC task request: %s" % (
                                 self.current_mode,
                                 self.state_names.get(self.state_machine_state, "UNKNOWN"),
                                 request.task_name))
    ...
    def mode_callback(self, msg):
        """Handle RobotMode messages (for backward compatibility)"""
        requested_mode = None
        if msg.data == 0:
            requested_mode = "MODE-MOVEIT"
        elif msg.data == 2:
    ...
    def is_safe_to_switch_mode(self, requested_mode):
        """
        Check if it's safe to switch modes based on current MTC state machine state
        """
        ...
        if self.state_machine_state == 3 and requested_mode == "MODE-MANUAL":  # ERROR state
            return True
            
        # Don't allow switching during task execution
        if self.state_machine_state == 1:  # EXECUTING_TASK state
        ...
        # Safe states for mode switching: IDLE, COMPLETED
        if self.state_machine_state in [0, 2]:  # IDLE, COMPLETED
            return True
    ...
    def state_machine_callback(self, msg):
        """Callback to track MTC state machine state for synchronization"""
        self.state_machine_state = msg.data
        self.get_logger().debug("MODE: %s -- STATE-MACHINE: %s -- MTC State machine state updated" % (
                              self.current_mode, 
                              self.state_names.get(self.state_machine_state, "UNKNOWN")))
    ...
    def switch_processes(self, mode):
        ...
        if mode == "MODE-MANUAL":
            # Graceful transition: wait if robot is executing task
            if self.state_machine_state == 1:  # EXECUTING_TASK state
                self.get_logger().info("MODE: %s -- STATE-MACHINE: %s -- Waiting for task execution to complete..." % (
                                     mode, 
                                     self.state_names.get(self.state_machine_state, "UNKNOWN")))
...
def main(args=None):
    ...
    mtc_mode_switcher = MTCModeSwitcher()
    
    rclpy.spin(mtc_mode_switcher)
    mtc_mode_switcher.destroy_node()
    rclpy.shutdown()
